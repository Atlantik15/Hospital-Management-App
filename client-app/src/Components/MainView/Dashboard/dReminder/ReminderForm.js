...
const ReminderForm = ({ classes, ...props }) => {
  ...
  const validate = (fieldValues = values) => {
    ...
    if ("reminderTitle" in fieldValues)
      temp.reminderTitle = fieldValues.reminderTitle
        ? ""
        : "This field is required.";
    if ("reminderDate" in fieldValues)
      temp.reminderDate = fieldValues.reminderDate
        ? ""
        : "This field is required.";
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      if (props.currentId == 0) props.createDReminder(values, onSuccess);
      else props.updateDReminder(props.currentId, values, onSuccess);
    }
  };
          ...
          <Grid item xs={4}>
            <Input
              ...
              {...(errors.reminderTitle && {
                error: true,
                helperText: errors.reminderTitle,
              })}
            />
          </Grid>
          <Grid item xs={4}>
