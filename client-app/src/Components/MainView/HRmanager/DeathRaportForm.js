...
const DeathRaportForm = ({ classes, ...props }) => {
  ...
  const validate = (fieldValues = values) => {
    ...
    if ("causeOfDeath" in fieldValues)
      temp.causeOfDeath = fieldValues.causeOfDeath
        ? ""
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      if (props.currentId == 0) props.createDeathRaport(values, onSuccess);
      else props.updateDeathRaport(props.currentId, values, onSuccess);
    }
  };
...
export default connect(
  mapStateToProps,
  mapActionToProps
)(withStyles(styles)(DeathRaportForm));
