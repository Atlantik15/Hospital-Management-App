...
const DoctorForm = ({ classes, ...props }) => {
  ...
  const validate = (fieldValues = values) => {
    ...
    if ("password" in fieldValues)
      temp.password =
        fieldValues.password.length > 8 ? "" : "Minimum 8 chars required";
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      if (props.currentId == 0) props.createDoctor(values, onSuccess);
      else props.updateDoctor(props.currentId, values, onSuccess);
    }
  };
