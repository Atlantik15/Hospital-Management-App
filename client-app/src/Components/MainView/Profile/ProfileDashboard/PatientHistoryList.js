...
export default observer(function PatientHistoryList() {
  ...
  const { deletePatientHistory, patienthistoriesByDescription, loading } =
    patientHistoryStore;
      ...
      <TableBody>
        {patienthistoriesByDescription.map((patientHistory) => (
          <TableRow key={patientHistory.id}>
            <TableCell style={{ width: "100px", color: "purple" }}>
              <b>{patientHistory.date}</b>
            </TableCell>
            <TableCell style={{ width: "300px" }}>
              {patientHistory.descritpion}
            </TableCell>
            ...
            <TableCell>
              {user.role === "admin" && (
                <ButtonGroup variant="text">
                  <Button
                    onClick={() =>
                      patientHistoryStore.selectPatientHistory(
                        patientHistory.id
                      )
                    }
                    basic
                  ...
                  <Button
                    name={patientHistory.id}
                    loading={loading && target === patientHistory.id}
                    onClick={(e) => handleHistoryDelete(e, patientHistory.id)}
                    basic
                    color="black"
                  >
